#!/usr/bin/env python3
"""
NLL Evaluation for Base Qwen3-8B Model

Computes Negative Log-Likelihood (NLL) and Perplexity on the govreport validation set
using the base Qwen/Qwen3-8B model (without DMS compression).

Usage:
    python evaluate_nll_qwen3.py --max_samples 10 --max_length 4096

Note: You may need to download the model first if it's not cached.
      Run with internet access: huggingface-cli download Qwen/Qwen3-8B
"""
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate NLL for base Qwen3-8B")
    ...
    parser.add_argument("--output_file", type=str, default="nll_results_qwen3.json",
                        help="Output file for results")
    args = parser.parse_args()

    print("=" * 60)
    print("NLL Evaluation: Qwen/Qwen3-8B (Base Model)")
    print("=" * 60)
    ...
    # Load model
    print("Loading base Qwen3-8B model...")
    print("NOTE: Model weights must be downloaded first if not cached.")
    print("      Run: huggingface-cli download Qwen/Qwen3-8B")
    ...
    # Print results
    print("\n" + "=" * 60)
    print("Results: Qwen/Qwen3-8B (Base Model)")
    print("=" * 60)
